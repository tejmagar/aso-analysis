    ...
    if dl_tr.sum() >= 50:
        ...
        if dl_ho.sum():
            ...
            dl.update(mae_log=float(np.abs(dm - dt).mean()),
                      ...
                      # Kept as a single figure, not a distribution. The full
                      # spread distribution was recorded here so serving could
                      # say whether a page's disagreement was unusual; that
                      # confidence was dropped once the spread turned out to
                      # correlate +0.02 with actual error, and the percentiles
                      # went with it rather than sitting in every checkpoint
                      # unread.
